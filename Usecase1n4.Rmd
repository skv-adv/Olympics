...
#Usecase 1
kaggle_data <-  read_excel("C:/Users/Dell/Desktop/Rutgers/DataWrangling/Project/athlete_events.xlsx")
#Usecase 1
...
```
```{r usecase6	Male vs Female participation over time in Olympics Medal summary}
rm(Medal_Summary_bynations)
Medal_Summary_bynations<-read_excel("C:/Users/Dell/Desktop/Rutgers/DataWrangling/Project/athlete_events.xlsx")
Medal_Summary_bynations <- Medal_Summary_bynations  %>%   filter(Medal != "NA")  %>%  na.omit()
Medal_Summary_bynations <- Medal_Summary_bynations %>%  
  filter(Sport != "Art Competitions")  %>%  dplyr::rename(region = Team)  %>%
  group_by(NOC, Medal, Sport, Year,Sex,region) %>% dplyr::summarise(isMedal=1) %>%
                           arrange(Medal,Year)

Medal_Summary_bynations <-  Medal_Summary_bynations %>% group_by(Medal,Sex,Year) %>%
                            dplyr::summarise(Count= sum(isMedal)) %>%  arrange(Year)


# TOTAL MEDAL COUNT for Male by Year over time
Medal_Summary_bynations_Male  <- Medal_Summary_bynations %>% filter(Sex == "M") %>%
  group_by(Medal,Year,Sex) %>% dplyr::summarise(Total=sum(Count)) %>% arrange(desc(Medal))
...
plot_Female <- ggplot(Medal_Summary_bynations_Female, aes(x=Year, y= Total,group=(Medal), color=Medal)) + geom_point(size=1.5) + geom_smooth(stat = "smooth") + labs(x = "Year", y = "Medal Count by Rank",title = "Number of Medals for Female Olympians over time") 
#+ theme(plot.title = element_text(hjust = 0.4)) 
#+  theme_linedraw() 
#+ scale_fill_distiller ()
plot_Female
